...
def main():
    ...
    # running peak over the window so far: highest close, its date, and the lowest implied
    # required return (which the peak close produces, since price and return move inversely)
    peak_i = max(range(len(bars)), key=lambda i: float(bars[i]["close"]))
    peak_b = bars[peak_i]
    peak_close = float(peak_b["close"])
    peak_high = max(float(b["high"]) for b in bars)
    peak_w = implied_pct(peak_close, sh)
    L = ["% Auto-generated by 10_fig_price_path.py; do not edit by hand.",
         ...
         f"\\newcommand{{\\ppPeakCumPct}}{{{(peak_close / offer - 1) * 100:.1f}}}",
         f"\\newcommand{{\\ppPeakWaccPct}}{{{peak_w:.2f}}}",
         f"\\newcommand{{\\ppPeakHigh}}{{{peak_high:.2f}}}",
         # latest close relative to the running peak (signed and magnitude), for the pullback prose
         f"\\newcommand{{\\ppFromPeakPct}}{{{(float(last_b['close']) / peak_close - 1) * 100:.1f}}}",
         f"\\newcommand{{\\ppFromPeakAbsPct}}{{{abs((float(last_b['close']) / peak_close - 1) * 100):.1f}}}"]
    if len(bars) >= 2:
        prev_c = float(bars[-2]["close"])
        latest_day_pct = (float(last_b["close"]) / prev_c - 1) * 100
        L += [f"\\newcommand{{\\ppSecondDayPct}}{{{(float(bars[1]['close']) / float(bars[0]['close']) - 1) * 100:.1f}}}",
              f"\\newcommand{{\\ppLatestDayPct}}{{{latest_day_pct:.1f}}}",
              # signed move's magnitude, for prose that supplies its own direction word
              f"\\newcommand{{\\ppLatestDayAbsPct}}{{{abs(latest_day_pct):.1f}}}",
